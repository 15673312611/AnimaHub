...
import { Sparkles, Wand2, Upload, Loader2, LayoutTemplate, X, UploadCloud } from "lucide-react";
  ...
  const handleSubmit = async () => {
    ...
    await safeAsync(
      async () => {
        ...
        return await api.post(`/projects/${projectId}/assets/effects/${endpoint}`, submitData);
      },
      toast,
      {
    ...
    <>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="bg-zinc-950/95 backdrop-blur-xl text-white border-white/10 max-w-4xl rounded-2xl shadow-2xl p-0 overflow-hidden max-h-[90vh] overflow-y-auto">
          <DialogHeader className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-pink-900/20 to-rose-900/20">
            ...
            <div className="p-5">
               {mode === "generate" ? (
                 <div className="space-y-4">
                    {/* AI 智能生成区域 */}
                    <div className="bg-gradient-to-r from-pink-500/10 to-rose-500/10 rounded-xl p-3 border border-pink-500/20">
                      <div className="flex gap-2 items-center">
                        ...
                        <Button 
                          ...
                          className="bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 h-9 px-4 rounded-lg text-xs"
                        >
...
{aiGenerating ? "生成中..." : "智能提示词"}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <Label className="text-xs text-zinc-500 mb-1.5 block">特效名称 *</Label>
                        <Input 
                          value={formData.name}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <Label className="text-xs text-zinc-500">特效描述 (Prompt)</Label>
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          onClick={insertTemplate}
                          className="text-[11px] text-pink-400 hover:text-pink-300 hover:bg-pink-500/10 h-6 px-2"
                        >
                      ...
                      <Textarea 
                         ...
                         className="bg-zinc-900/30 border-white/10 min-h-[200px] text-sm resize-none rounded-xl focus:border-pink-500/50 focus:ring-1 focus:ring-pink-500/20 font-mono"
                       />
                    </div>
                 </div>
               ) : (
            ...
            <DialogFooter className="px-6 py-4 bg-zinc-900/50 border-t border-white/5">
              ...
              <Button onClick={handleSubmit} disabled={creating} className="bg-gradient-to-r from-pink-600 to-rose-600 text-white h-10 px-6 rounded-xl shadow-lg">
